...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) > 1 and len(grid.get_cells()[i][j]) < min_domain_size:
                    min_domain_size = len(grid.get_cells()[i][j])
                    min_domain = (i, j)
        return min_domain
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        # save all cell indicies with only one value in their domain
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) == 1:
                    Q.add((i, j))
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q) > 0:
            ...
            variables_assigned_row, failure_row = self.remove_domain_row(grid, row, col)
            
            # remove value from column
            variables_assigned_col, failure_col = self.remove_domain_column(grid, row, col)

            # remove value from unit
            variables_assigned_unit, failure_unit = self.remove_domain_unit(grid, row, col)
            
            if failure_row or failure_col or failure_unit:
                return True

            # add variables that had their domains reduced to size 1 to Q
            for var in variables_assigned_row:
                Q.add(var)
            for var in variables_assigned_col:
                Q.add(var)
            for var in variables_assigned_unit:
                Q.add(var)
...
class Backtracking:
    ...
    def __init__(self, grid):
        self.grid = grid
        self.ac3 = AC3()
        self.Q = self.ac3.pre_process_consistency(self.grid)
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[var[0]][var[1]]:            
            # check if d is consistent with A
            if not grid.is_value_consistent(d, var[0], var[1]):
                continue

            # set the value in the copy of the grid
            copy_grid = grid.copy()
            copy_grid.get_cells()[var[0]][var[1]] = d
            
            inconsistency_found = self.ac3.consistency(copy_grid, self.Q)

            if inconsistency_found:        
                # backtrack again in the updated grid
...
for p in problems:
    ...
    # mrv = MRV()
    # solution = backtracking.search(g.copy(), mrv)

    backtracking2 = Backtracking(g)
    firstavailable = FirstAvailable()
    solution = backtracking2.search(g.copy(), firstavailable)
    # print("Solution for backtracking using FirstAvailable: ", solution.is_solved())
    ...
    # firstavailable = FirstAvailable()
    # solution = backtracking.search(g.copy(), firstavailable)
    # print("Solution for backtracking using FirstAvailable: ", solution.is_solved())
